...
describe("titleRegionConditions — 태그 없는 제목의 시군구 사전", () => {
  ...
  it("리뷰 지적 2: 기관에 시도가 없으면(수원도시재단) 아무 조건도 만들지 않는다 — 서울 회사를 지우지 않고 지역 칸 폴백도 막지 않는다", () => {
    expect(titleRegionConditions("[시설·공간 ] 수원시 기업지원센터 1인창조기업 입주모집 공고", "수원도시재단")).toEqual([]);
  });

  it("리뷰 지적 2: 제목에 「전국」이 있으면 기관이 검증돼도 시군구는 장소일 뿐 — 조건 없음", () => {
    expect(titleRegionConditions("전국 소상공인 창원시 박람회 참가기업 모집 공고", "경상남도")).toEqual([]);
  });
