import { extractConditions, titleRegionConditions, titleSectorCondition, titleTargetOrgCondition } from "../engine/rule-extract";
 ...
 *  6: (가지 안에서만 존재한 판 — 배포되지 않았다)
 *  7: 대상 유형(자격) targetOrg 조건(2026-09-17 — 사회적기업·착한가격업소 같은 자격형 공고가 지역 pass 하나로 맞음이 되던 것)(2026-09-17 — 표본 화면 419건 중 자격형 22건이 지역 pass 하나로 맞음이 됐다.
 *     사회적기업·착한가격업소·예비창업자는 규모가 아니라 자격이다)
 */
export const RULE_EXTRACT_VERSION = 7;
...
export function buildRuleStructure(
  ...
  // 대상 분야·대상 유형은 제목이 못 박았을 때만(본문에서 뽑으면 지나가는 말이 fail 이 된다).
  const sector = titleSectorCondition(title);
  const org = titleTargetOrgCondition(title);
  const conditions = [
    ...body,
