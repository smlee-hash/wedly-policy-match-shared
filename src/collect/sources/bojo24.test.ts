...
describe("보조금24 — 원본이 「개인」이라 해도 사업주 공고면 담는다", () => {
  ...
  // ★2026-09-23 누락 실측 — 「농가」 앞을 막았더니 「양봉농가·축산농가」 같은 합성어가 전부 빠졌다
  //  (원본 개인 표시 93건: 양봉농가 구제약품·축산농가 톱밥·양돈농가 백신·벼 재배농가 …).
  it.each([
    ["양봉농가", { 서비스명: "양봉농가 구제약품지원", 지원대상: "관내 양봉농가" }],
    ["축산농가 붙여 씀", { 서비스명: "축산농가지원", 지원대상: "" }],
    ["재배농가", { 서비스명: "벼 재배농가 동력살분무기 지원", 지원대상: "" }],
    ["한돈농가", { 서비스명: "한돈농가 인공수정(정액) 지원", 지원대상: "" }],
    ["꿀벌사육농가", { 서비스명: "꿀벌사육농가 화분 지원", 지원대상: "" }],
  ])("합성어 「…농가」 공고는 담는다 — %s", (_l, over) => {
    expect(passes(q(over))).toBe(true);
  });

  // ★리뷰 review-0f08ca4d — 「학비」가 「화학비료」에, 「농가」가 「영세농가의 자녀」에 걸리던 것
  it("「화학비료」 공고는 「학비」 복지로 보지 않는다", () => {
    expect(passes(q({ 서비스명: "화학비료 구입비 지원", 지원대상: "관내 농업경영체로 등록한 농업인" }))).toBe(true);
  });
  // ★리뷰 review-c3f06329 — 「과학비즈니스」, 「영세농가의 대학생 자녀」
  it("「과학비즈니스벨트」 창업 공고는 「학비」 복지로 보지 않는다", () => {
    expect(passes(q({ 서비스명: "과학비즈니스벨트 창업지원", 지원대상: "예비창업자 및 창업 3년 이내 기업" }))).toBe(true);
  });
  // ★리뷰 review-94cbbf54 — 예외 규칙이 사업주 공고를 버리던 두 경우. 예외를 없애 담는다.
  it("「농가 경영비 지원」 은 다른 칸에 「자녀」가 있어도 담는다", () => {
    expect(passes(q({ 서비스명: "농가 경영비 지원", 지원대상: "자녀를 양육하는 가구 우대" }))).toBe(true);
  });
  it("「영농기술 교육 학비」 는 농업인 공고로 담는다", () => {
    expect(passes(q({ 서비스명: "농업인 영농기술 교육 학비 지원", 지원대상: "농업경영체를 운영하며 영농기술 교육과정을 수강하는 농업인 본인" }))).toBe(true);
  });
  it("「농가 자녀 영농 승계」 는 영농 신호로 담는다", () => {
    expect(passes(q({ 서비스명: "농가 자녀 영농 승계 지원", 지원대상: "영농 승계 희망 농가 자녀" }))).toBe(true);
  });
