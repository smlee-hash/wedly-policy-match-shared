 ...
 *
 * `어가`·`어선` 은 앞뒤를 막았다 — 안 막으면 「만들**어가**는」·「되**어선**」에 걸려 순수 복지 공고가
 * 담긴다(적대 리뷰 ⑤ 실측: 「행복 돌봄 지원」·「아이 돌봄 서비스」).
 * `농가` 는 막지 않는다(2026-09-23 누락 실측) — 앞을 막았더니 「양봉농가·축산농가·벼 재배농가」 같은
 * 합성어 93건이 전부 빠졌다. 「농가의 자녀」 같은 배경 표현을 빼려는 예외는 리뷰 3회 동안 매번 사업주
 * 공고(「농가 경영비 지원(자녀 양육 가구 우대)」·「영농기술 교육 학비」)를 새로 버려서 두지 않는다 —
 * 사장님 기준대로 넓게 담고 걸러내기는 매칭에서 한다(학생 복지 1~2건이 섞이는 쪽을 택함).
 */
const PRIMARY_SIGNAL =
  /농업인|어업인|임업인|축산업|영농|영어조합|양식업|귀농|귀어|귀산|농가|(?<![가-힣])(?:어가|어선)(?![가-힣])|농어업|농어촌|어촌|농촌|산촌|수산업경영인|산림경영|농업경영체|어업경영체/;
