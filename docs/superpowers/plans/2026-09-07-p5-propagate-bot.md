...
14. **2026-09-08 4차 독립 리뷰(Astra) 3건(전부 P2) — 판정대로 구현했다.**

    | # | 무엇이 문제였나 | 어떻게 닫았나 |
    |---|---|---|
    | **H1(P2)** | 봉인 임시 폴더를 `dir="$(crypto_dir)"` 로 만들었다. **명령 치환은 하위 셸**이라 거기서 정한 `CRYPTO_DIR` 이 부모에 남지 않아 EXIT 갈고리의 `cleanup` 이 지울 자리를 몰랐다 — 복호가 실패한 순간(짝이 아닌 비밀키·깨진 암호문·tar 오류·링크 거부) **비밀키 `priv.pem` 과 1회용 열쇠 `session.key` 가 임시 폴더에 그대로 남았다**(옛 코드로 되돌려 네 경우 모두 실측 재현) | `crypto_dir` 을 **부모 셸에서** 부르고 경로는 전역 `$CRYPTO_DIR` 로 읽는다(`lib.sh` 의 봉인·해제 두 자리). 시험 4건은 `TMPDIR` 를 시험 폴더로 못 박고 **종료 뒤 그 폴더를 통째로 훑어** `propagate-crypto.*`·`*.pem`·`session.key`·「PRIVATE KEY」 0건을 잰다 |
    | **H2(P2)** | 두 대조기가 값은 `shown` 으로 가려 찍으면서도 **읽다 죽은 자리에서는 `err.message` 를 그대로** 적었다. `JSON.parse` 의 오류문에는 **입력 앞부분이 들어간다**(node 22 실측: `Unexpected token 'P', "PRIVATE_RE"... is not valid JSON`) — 이 저장소는 공개라 Actions 로그도 공개고, 대조기가 읽는 것은 **봉인해서 온 비공개 앱 파일**이라 깨진 파일 하나면 그 앞부분이 공개 로그에 실렸다 | 세 자리(`verify-lock` readJson · `verify-artifact` readJson · 등록부 JSON 검사)에서 **읽기와 해석을 따로 감싸고**, 구문 오류만 「JSON 구문 오류(내용은 표시하지 않음) — <파일 이름>, <길이>바이트」 **고정 문구**로 적는다(파일 없음·보통 파일 아님 같은 나머지 오류문에는 내용이 안 들어가 그대로 둔다). 시험은 표식 `PRIVATE_REVIEW_CANARY_NOT_A_SECRET` 를 담은 깨진 JSON 을 산출물·기준·설치 잠금·등록부에 넣어 stdout·stderr 어디에도 표식과 **그 앞 9글자**가 없음을 잰다(V8 이 10여 글자에서 자르므로 표식 전체만 보면 껍데기 시험이 된다) |
    | **H3(P2)** | 봉인 tar 안의 `meta.json` 이 **이름있는 통로(FIFO)** 면 「푼 뒤 심볼릭 링크만 보기」를 지나갔다. 밀기의 `[ -f ]` 가 잡아 exit 1 로 끝나도 **FIFO 는 그 자리에 남고**, 그때 도는 워크플로우의 「실패 알림」이 `readFileSync` 로 그것을 열다가 **영원히 멈춘다**(읽는 쪽만 있고 쓰는 쪽이 없다 — 실측) → 알림이 안 가고 job 이 제한 시간(65분)까지 매달린다 | ① 풀기 **전에** `tar -tzvf` 목록을 보고 보통 파일(`-`)·폴더(`d`) 말고 다른 항목(링크 `l`·`h`·FIFO `p`·장치 `b`/`c`)이 있으면 **풀지 않고** exit 1 ② 푼 뒤에도 `find ! -type f ! -type d` 로 전 항목 재검사 ③ 워크플로우의 실패 알림은 읽기 전에 `lstat` 으로 **정규 파일인지** 보고 아니면 기본 사유로 알린다. 오류문에는 **목록·이름을 싣지 않는다**(개수만 — F9·H2 의 로그 규칙). 시험 2건: FIFO `meta.json` 을 담은 봉인으로 push → exit 1 이고 **푼 자리가 아예 안 생긴다** / 알림 셸 토막(`workflow-shell.test.ts` 방식으로 추출)이 FIFO 앞에서 안 멈추고 **5초 안에** 기본 사유로 `notify` 를 부른다(가짜 `curl`) |

    셋 다 **옛 코드로 되돌려 빨개지는 것을 확인**한 뒤 되돌렸다(H2 는 세 자리 각각, H3 는 두 자리 각각).

    **★공개 로그 노출 — 감수한 것(2026-09-08 5차 리뷰 · 총괄 결정)**
    이 저장소는 공개라 Actions 로그도 슬랙 알림 줄도 공개다. 5차 리뷰가 훑은 「로그로 나가는 값」 가운데
    **두 자리는 고쳤고**(앱 `package.json` 의 형식 어긋난 **핀 값** → `shown_pin` 으로 길이만,
    워크플로우 실패 알림의 **사유** → 밀기 스크립트 `meta_error` 와 같은 제어문자·이스케이프 지우기),
    나머지 셋은 **고치지 않고 감수한다**: ① 준비 로그의 **STRAY 파일 이름 목록** — 그 job 은 앱의
    npm·후처리 출력이 이미 통째로 공개 로그에 찍히는 자리라 파일 **이름 수준**은 새로 여는 구멍이 아니고,
    무엇이 왜 바뀌었는지 사람이 알 유일한 단서다(이름만 찍고 내용은 안 찍는다). ② `SUBJECT` — **공개**
    패키지 저장소의 커밋 제목이라 애초에 비공개가 아니다(커밋 제목으로도 그대로 들어간다).
    ③ `PROPAGATE_SHA`·단계 이름·`APP_ID` 원문 — 워크플로우가 정해 넣는 **우리 쪽 입력**이지 남이 고른
    글자가 아니다. 이 셋이 바뀌면(예: 준비 로그가 파일 **내용**까지 찍게 되면) 감수는 다시 판단한다.
