...
## 진행 기록 (2026-09-24)
- 구현: 관리형 실행기(Grok 공유 한도 소진 → Opus) 3묶음. 총괄이 실사이트로 돌려 찾아 고친 것: ksure 닫히지 않은 span(매입·포괄매입 본문 빈 값), 서울·경기 한도 숫자(가장 큰 값 → 대표 금액 `guarantee-limit.ts`).
- 독립 리뷰 1차 P1(대상 글 비면 지역만으로 맞음)·P2(600자 잘림) 고침(`guarantee-target.ts`). 2차 통과.
- **알려진 한계:** 자금 지도 조립(`src/build/funding-map-build.ts:473`)이 모든 상품의 대상 글을 600자로 자른다(이번 변경 전부터). 판정은 「확인 필요」로 유지되지만 상세 창에 긴 원문이 다 안 보인다 — 원문 링크로 확인. 푸는 것은 지도 9화면 표시 변경이라 별도 작업.
- 실사이트 수집(2026-09-24 로컬, NODE_EXTRA_CA_CERTS 포함): 무역보험 5·서울신보 9·경기신보 7.
- 배포는 미리보기(https://share.onorca.dev/a/fUkrPo0nlMzw) 승인 뒤.
