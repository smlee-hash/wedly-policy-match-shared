...
**밀기 걸음은 준비 걸음의 결과를 「믿을 수 없는 자료」로 다룬다**(2026-09-08 2차 검토). 준비 걸음에서는
앱의 `npm` 스크립트와 그 의존성이 도는데, 그 코드는 꾸러미(artifact) 안의 파일을 마음대로 고칠 수 있다.
그래서 밀기 걸음은 **`resolve` 가 정한 커밋 하나만** 믿고 나머지는 자기 눈으로 다시 본다.

- **어느 커밋으로 가는지** — 꾸러미가 말하는 SHA 가 이번 반영 커밋과 같은지, 앱의 지금 핀이 정말 그것인지,
  그 핀이 이번 커밋의 **조상**인지(핀이 뒤로 가지 않게)를 밀기 걸음이 직접 확인한다.
- **파일 안에 무엇이 들었는지** — `scripts/propagate/verify-artifact.mjs` 가 **앱 저장소의 원래 파일에서
  「핀 한 줄만 바꾼 것」을 만들어 통째로 대조**한다. 그래서 `package.json` 에 `postinstall` 이 하나 붙거나,
  잠금 파일의 `resolved` 가 남의 주소로 바뀌거나, 모르는 꾸러미 항목이 끼어들면 **아무것도 밀지 않는다.**
- **파일을 어디에 쓰는지** — 덮어쓸 자리와 그 위 폴더가 전부 진짜 파일·폴더인지 본다. 심볼릭 링크면 멈춘다
  (링크를 따라가 클론의 `.git/config` 를 덮으면 그다음 git 호출에서 열쇠가 샌다).

**꾸러미(artifact)는 봉인돼 올라간다.** 이 보관함은 **공개**라 GitHub Actions 가 남기는 꾸러미는
누구나 내려받을 수 있는데, 그 안에는 **비공개** 앱 3곳의 `package.json`·잠금 파일·설계 등록부가 들어간다
(= 그 앱이 쓰는 꾸러미 목록과 사내 주소가 통째로 공개된다). 그래서 준비 걸음은 파일을 그대로 올리지 않고
**공개키로 잠근 두 장**(`bundle.enc`·`key.enc`)만 올리고, 밀기 걸음만 비밀키로 그것을 연다.
공개키(변수 `PROPAGATE_ARTIFACT_PUBKEY`)가 비어 있으면 준비 걸음은 **아무것도 올리지 않고 실패한다** —
실수로라도 평문이 올라가는 길을 두지 않는다. 준비가 죽었을 때의 **실패 사유도 같은 봉인**을 거치고,
봉인이 안 되면 그것마저 올리지 않는다(그때 밀기 걸음이 「꾸러미를 받지 못했다」로 빨갛게 끝나 알림이 간다).
잠그는 열쇠는 **누구나 아는 공개키**라 봉인은 「엿보기」만 막고 위조는 막지 못한다 — 그래서 밀기 걸음은
봉인을 푼 **뒤에도** 위에 적은 대조를 전부 다시 한다. 같은 이유로 **로그에도 파일 내용을 찍지 않는다**
(Actions 로그도 공개다 — 어긋난 자리의 **이름과 개수**만 적는다).

**★봇이 「모르는 꾸러미 항목」으로 멈추면** — 그건 고장이 아니라 설계다. 이유는 둘 중 하나다.

1. **이 보관함이 런타임 의존을 새로 더했다**(`dependencies` 에 추가) → 앱 잠금 파일에도 그 꾸러미가 새로 생긴다.
   사람이 §4 대로 한 번 손으로 올리고, 그 의존을 앱에 들여도 되는지 판단한 뒤 규칙을 넓힐지 정한다.
2. **그 앱의 잠금 파일이 `npm` 판과 어긋나 있다** → 핀과 상관없이 `npm install --package-lock-only` 만 돌려도
   항목이 늘거나 준다. 그 앱에서 그 명령을 한 번 돌려 **사람이 커밋**해 두면 그다음부터 조용해진다.
   (2026-09-08 실측: 랩 저장소가 이 경우 6건, ERP·일루아는 0건.)
...
- **실패를 조용히 넘기지 않는다.** 어느 걸음에서 죽든 슬랙(ERP 내부 알림 통로)으로 **한 번** 알린다. 그때 그 앱의 `main` 은 **옛 핀 그대로**다 — 반쯤 반영된 상태가 남지 않는다.

**알림은 밀기 걸음 한 곳에서만 보낸다.** 준비 걸음은 앱 코드가 이미 한 번 돈 자리라, 거기서 알림 열쇠를
쓰면 **바꿔치기한 알림 스크립트 하나로 열쇠가 샌다.** 그래서 준비 걸음에는 알림이 없고(열쇠도 없고),
준비가 죽으면 **그 사유를 꾸러미에 적어** 보낸다. 밀기 걸음이 그 사유를 읽어 빨갛게 끝내고 슬랙 글에
그대로 적는다 — 그래서 「준비가 죽어 꾸러미가 없는 실패」도 빠짐없이 알려진다.
...
**이 스위치는 「CI 성공 뒤 자동으로 도는 것」만 막는다** — 꺼 둔 동안에도 아래 「예행연습」처럼 **손으로
돌리는 것은 된다**(그래야 처음 켜기 전에 예행으로 확인할 수 있고, 꺼 둔 동안 급한 한 앱을 손으로 맞출 수 있다).
...
| 시크릿 `PROPAGATE_ARTIFACT_PRIVKEY` | 꾸러미 봉인을 **푸는** 비밀키(PEM) · 밀기 걸음에만 쓴다 | 준비까지는 되고 밀기가 봉인을 못 열어 실패한다 |
| 변수 `WEDLY_NOTIFY_URL` | ERP 주소 | 위와 같다 |
| 변수 `PROPAGATE_ENABLED` | 끄는 스위치 | 켜진 것으로 본다 |
| 변수 `PROPAGATE_ARTIFACT_PUBKEY` | 꾸러미를 **봉인하는** 공개키(PEM) · 비밀이 아니라서 변수다 | 준비 걸음이 **아무것도 올리지 않고** 실패한다(아래 「꾸러미는 봉인돼 올라간다」) |
...
**봉인 열쇠쌍은 첫 가동 전에 사람이 한 번 만든다.** RSA 2048 키쌍을 만들어 **공개키는 변수**
`PROPAGATE_ARTIFACT_PUBKEY` 에, **비밀키는 시크릿** `PROPAGATE_ARTIFACT_PRIVKEY` 에 넣는다(둘 다 PEM 글자 그대로).
공개키는 감출 것이 아니지만 비밀키는 **이 보관함 시크릿과 사람의 열쇠 보관 자리 밖으로 내보내지 않는다.**
바꿀 때는 **공개키를 먼저** 바꾸고 그다음 비밀키를 바꾼다(반대로 하면 이미 올라간 꾸러미를 못 연다).
...
**동시에 여러 번 돌면.** 줄이 셋으로 나뉜다 — 자동 반영은 `propagate-main`, 손으로 돌린 **실제 반영**은
`propagate-manual`, 손으로 돌린 **예행**은 `propagate-dry`. 예행이 **대기 중인 진짜 반영을 취소하지 못하게**
갈라 둔 것이다. 한 줄에서 대기 중인 실행은 GitHub 가 최신 것 하나만 남기는데, `main` 은 직선이라
최신 커밋이 앞 커밋을 포함하므로 결과는 같다.

**늦게 끝난 옛 CI 는 최신 핀을 묻지 못한다.** 옛 커밋의 CI 가 한참 뒤에 끝나 그때서야 반영이 도착하면,
그 실행이 최신 커밋의 대기 실행을 밀어내고 **한 커밋 뒤에서 멈출** 수 있다. 그래서 봇은 그럴 때
그 커밋 **뒤에 온 커밋들을 최신부터(최대 30개) 훑어** CI 를 통과한 첫 커밋으로 올려 반영한다.
끝 커밋 하나만 보면, 그 커밋의 CI 가 아직 안 끝났거나 빨간 동안 **그 앞의 통과한 커밋을 아무도 반영하지
못한다.** 물어보다 실패하거나 통과한 커밋이 없으면 원래 커밋 그대로 간다 — 확인 실패가 반영을 막지는 않는다.
