...
**★언젠가 봇이 「모르는 꾸러미 항목」으로 멈추면** — 그건 고장이 아니라 설계다. 이 보관함이 **런타임 의존을
새로 더하면**(`dependencies` 에 무언가 추가) 앱 잠금 파일에도 그 꾸러미 항목이 새로 생기는데, 위 대조기는
「핀 한 줄만 바뀐 것」만 받으므로 그때 안전하게 실패한다. 그때는 사람이 §4 대로 손으로 한 번 올리고,
그 의존을 앱에 들여도 되는지 판단한 뒤 대조기 규칙을 넓힐지 정한다.
